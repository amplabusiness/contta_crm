...
import { mapEmpresaRecordToResponse, sanitizeCnpj } from '../utils/formatters.ts';
...
export default async function handler(
  ...
  try {
    ...
    response.status(200).json(mapEmpresaRecordToResponse(data));
  } catch (err: any) {
    console.error('Error in empresa detail API:', err);
